...
CREATE TABLE IF NOT EXISTS sa_invoices (
  ...
  invoice_number      TEXT UNIQUE,                 -- Human-readable invoice #, key for QB matching (TEXT to match qb_invoices.invoice_number; UNIQUE required for FK target)
...
-- Uniqueness is enforced via two partial indexes below to handle nullable identifiers correctly.
CREATE TABLE IF NOT EXISTS sa_payment_applications (
  id               BIGSERIAL PRIMARY KEY,
  payment_sa_id    TEXT NOT NULL REFERENCES sa_payments(sa_id) ON DELETE CASCADE,  -- FK to sa_payments.sa_id
  invoice_number   TEXT,                           -- FK to sa_invoices.invoice_number (TEXT to match sa_invoices; nullable when only invoice_sa_id is known)
...
CREATE TABLE IF NOT EXISTS audit_matches (
  id                    BIGSERIAL PRIMARY KEY,
  sa_invoice_sa_id      TEXT REFERENCES sa_invoices(sa_id) ON DELETE SET NULL,
  qb_invoice_id         TEXT REFERENCES qb_invoices(qb_id) ON DELETE SET NULL,  -- Enforced FK; was comment-only before
...
-- Partial unique indexes for sa_payment_applications deduplication.
-- Two separate indexes are required because NULLs are never equal in a standard UNIQUE
-- constraint, which would allow unlimited duplicate rows when either identifier is NULL.
-- Each index covers exactly the rows where that identifier is populated, making
-- ON CONFLICT targeting unambiguous for upsert logic.
CREATE UNIQUE INDEX IF NOT EXISTS ux_pay_apps_by_number
  ON sa_payment_applications(payment_sa_id, invoice_number)
  WHERE invoice_number IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ux_pay_apps_by_sa_id
  ON sa_payment_applications(payment_sa_id, invoice_sa_id)
  WHERE invoice_sa_id IS NOT NULL;

-- Row Level Security
-- Enabled on all tables containing customer PII and financial data.
-- With RLS enabled and no permissive policies defined, Postgres defaults to
-- deny-all for the anon and authenticated roles, preventing accidental exposure
-- via the auto-generated PostgREST API. The service_role key bypasses RLS and
-- retains full access for trusted backend/sync processes.
-- Add explicit, narrowly-scoped policies here if authenticated users require
-- direct table access (e.g., an internal dashboard).
ALTER TABLE sa_invoices              ENABLE ROW LEVEL SECURITY;
ALTER TABLE sa_payments              ENABLE ROW LEVEL SECURITY;
ALTER TABLE sa_payment_applications  ENABLE ROW LEVEL SECURITY;
ALTER TABLE qb_invoices              ENABLE ROW LEVEL SECURITY;
ALTER TABLE qb_payments              ENABLE ROW LEVEL SECURITY;
ALTER TABLE audit_matches            ENABLE ROW LEVEL SECURITY;
